  ...
  return (
    <section className='bg-indigo-600 px-5 py-14 xl:px-32'>
      <div className="flex items-center justify-between">
        ...
        <Link href='/news' className="border-2 border-white py-2 px-6 text-white text-sm hover:text-indigo-600 hover:bg-white hover:duration-150">
          Selengkapnya
        </Link>
      </div>
      <div className='mt-2'>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-8">
        <Link href={`/news/${latestArticle.url}`} className="relative group h-[20rem] rounded-2xl overflow-hidden lg:h-[24rem]">
        ...
        <section>
          ...
          {moreArticle.map((article) => (
            <Link key={article.hash} href={`/news/${article.url}`} className="grid grid-cols-3 gap-3 mb-3 group">
