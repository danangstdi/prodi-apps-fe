...
export default async function News() {
  ...
  const moreArticle = articles.data.slice(1, 4)
  ...
  return (
    <section className='bg-indigo-600 px-5 py-14 xl:px-32'>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-8">
        <Link href='' className="relative group h-[20rem] rounded-2xl overflow-hidden lg:h-[24rem]">
          ...
          <div className="absolute bg-gray-900 bg-opacity-50 backdrop-blur-sm bottom-0 inset-x-0 text-white pt-5 pb-2 px-5">
            ...
            <div className="text-xs mt-4">
              {latestArticle.created_at.full}
            </div>
          </div>
        </Link>
        <section>
          <h5 className="text-white text-xl font-bold my-3 lg:hidden">Berita lainnya</h5>
          {moreArticle.map((article, index) => (
            <Link key={index} href='' className="grid grid-cols-3 gap-3 mb-3 group">
              <div className="col-span-1">
                <img src={article.thumbnail} className="w-full rounded-md object-cover" />
              </div>
              <div className="text-white col-span-2">
                <p className="line-clamp-3 font-bold group-hover:underline group-hover:underline-offset-2">
                  {article.title}
                </p>
                <p className="text-xs mt-2 text-slate-200">
                  {article.created_at.full}
                </p>
              </div>
            </Link>
          ))}
