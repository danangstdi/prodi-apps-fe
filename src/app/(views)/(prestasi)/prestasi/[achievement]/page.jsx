...
export async function generateMetadata({ params: { achievement } }) {
  const articleData = await getAchievement('/' + achievement);
...
export default async function Page({params: {achievement}}) {
  const getArticleByParam = await getAchievement('/' + achievement);
  const moreArticle = await getAchievement('?limit=4');
